OpenReview ID: mWBxEAq9lv
Submission number: 9121
Title: Approximation Error Upper and Lower Bounds for Hölder Class with Transformers
Live claim count / maximum points: 4 / 8
Selection timestamp: 2026-08-01T08:30:19Z
Contract manifest SHA-256: 877766a40afa82b81dc64c97d488759d4d59fde5fec17010b866c3ee1dce7c57
Source paper/version: arXiv:2605.07463, source archive and PDF SHA-256 in evidence/source/SHA256SUMS
Official code/data/model pins: no author executable located during source archive audit; clean-room direct reproduction required
Compute policy: cpu-upgrade default; no HF Job launched
GitHub repository: pending creation
Current phase: Claim 1 Attempt 1 source/CPU audit complete
Per-claim state: C1 inconclusive (source/rate audit only); C2-C4 not started
Publication status: not eligible; no points claimed
Selection rationale: Anchored live contract has four independently inspectable theorem/rate claims (8-point ceiling). Primary arXiv source archive and PDF downloaded successfully, theorem source is available, and direct numerical/constructive low-dimensional experiments are CPU feasible. Chosen over high-nominal candidates whose contracts require unavailable large-model/RL/video experiments.
